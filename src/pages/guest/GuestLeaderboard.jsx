...
import { Trophy, Star, ShieldCheck, Wrench, User, Minus, TrendingUp, TrendingDown } from 'lucide-react';
    ...
    <div className="page-enter">
      ...
      <div className="grid" style={{ gridTemplateColumns: '1.2fr 0.8fr', gap: 16, marginBottom: 20, marginTop: 16 }}>
        <div className="card" style={{ overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
          ...
          <div style={{ display: 'grid', gap: 10, padding: '16px', flex: 1 }}>
            {loading ? (
                  ...
                  <div key={leader.id} className="card" style={{ padding: '14px 16px', borderLeft: `4px solid ${borderLeftColor}`, display: 'flex', gap: 16, alignItems: 'center' }}>
                    ...
                    <div className="flex items-center justify-between" style={{ flex: 1 }}>
                      ...
                      <div style={{ textAlign: 'right', display: 'flex', alignItems: 'center', gap: 16 }}>
                        ...
                        {/* Trend indicator */}
                        <div style={{ paddingLeft: 12, borderLeft: '1px solid var(--border-dim)', display: 'flex', alignItems: 'center', justifyContent: 'center', height: '100%', width: 36 }}>
                          {leader.trend === 'up' && <TrendingUp size={20} color="var(--green-400)" title="Rank increased" />}
                          {leader.trend === 'down' && <TrendingDown size={20} color="var(--red-400)" title="Rank decreased" />}
                          {leader.trend === 'new' && <span style={{ color: 'var(--green-400)', fontSize: '0.7rem', fontWeight: 800, textTransform: 'uppercase' }}>NEW</span>}
                          {leader.trend === 'same' && <Minus size={18} color="var(--text-muted)" title="No change in ranking" />}
                        </div>
                      </div>
                    </div>
                  </div>
