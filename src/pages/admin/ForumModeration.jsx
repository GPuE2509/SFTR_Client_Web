      ...
      <div style={{ padding: '16px 20px' }}>
        ...
        <div className="flex items-start justify-between gap-4">
                ...
                <div style={{ marginTop: 12, padding: '10px 12px', background: 'rgba(255, 59, 48, 0.08)', borderRadius: 'var(--radius-sm)', border: '1px solid rgba(255, 59, 48, 0.2)' }}>
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6, color: 'var(--red-500)', fontSize: '0.85rem', fontWeight: 600 }}>
                      <Flag size={14} /> ⚠️ {post.reportCount} reports
                    </div>
                  ...
                  {reportsExpanded && post.reports && post.reports.length > 0 && (
                    <ul style={{ margin: '8px 0 0 0', paddingLeft: 20, color: 'var(--red-400)', fontSize: '0.8rem', lineHeight: 1.5, maxHeight: 120, overflowY: 'auto' }}>
                      {post.reports.map((r, idx) => (
                        <li key={idx} style={{ marginBottom: 4 }}>
                          <strong>{r.reason}</strong> {r.details ? `- ${r.details}` : ''}
                          <span style={{ fontStyle: 'italic', color: 'var(--red-300)', marginLeft: 4 }}>
                            (by {r.reporter_id?.full_name || 'Anonymous User'})
                          </span>
                        </li>
                      ))}
                    </ul>
          ...
          <div className="flex items-center gap-2" style={{ flexShrink: 0 }}>
            ...
            {post.status !== 'pending' && activeTab !== 'pinned' && (
        ...
        {showComments && (
          <div style={{ marginTop: 16, paddingTop: 16, borderTop: '1px solid var(--border-color)' }}>
            <h4 style={{ fontSize: '0.9rem', fontWeight: 600, color: 'var(--text-primary)', marginBottom: 12 }}>Comments</h4>
...
function CommentCard({ comment, onDelete, onDismiss }) {
  const [reportsExpanded, setReportsExpanded] = useState(false);
  ...
  return (
    <div className="card p-4" style={{ display: 'flex', gap: 16, marginBottom: 12, border: comment.reportCount > 0 ? '1px solid rgba(255, 59, 48, 0.3)' : 'none', position: 'relative', marginLeft: comment.isReply ? 40 : 0 }}>
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        <div style={{ color: 'var(--text-secondary)', fontSize: '0.95rem', lineHeight: 1.5, whiteSpace: 'pre-wrap' }}>
          {comment.content}
        </div>
...
export default function ForumModeration() {
  ...
  }, [posts, comments]); // It's fine to just update the ref on every render
    ...
    try {
      ...
      if (res && res.success) {
        const mapped = res.data.map(p => ({
          ...
          hearts: Array.isArray(p.hearts) ? p.hearts.length : 0,
  ...
  const [postFilter, setPostFilter] = useState('all'); // all, reported
  const [commentFilter, setCommentFilter] = useState('all'); // all, reported
  ...
  const filtered = posts.filter((p) => {
    if (activeTab === 'queue') return p.status === 'pending';
    if (activeTab === 'posts') {
      if (p.status !== 'approved') return false;
      if (postFilter === 'reported') return p.reportCount > 0;
      if (postFilter === 'reported_comments') {
        const pComments = comments.filter(c => c.postId === p.id);
        return pComments.some(c => c.reportCount > 0);
      }
      return true;
    }
  ...
  return (
    <div className="page-enter">
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: 16, marginBottom: 24 }}>
        {[
          { label: "Queue", value: posts.filter(p => p.status === 'pending').length, color: 'var(--orange-400)' },
          { label: "Posts", value: posts.filter(p => p.status === 'approved').length, color: 'var(--green-400)' },
          { label: "Pinned Posts", value: posts.filter(p => p.isPinned).length, color: 'var(--blue-400)' },
          { label: "Reported Posts", value: posts.filter(p => p.reportCount > 0).length, color: 'var(--red-400)' },
          { label: "Reported Comments", value: comments.filter(c => c.reportCount > 0).length, color: 'var(--red-500)' },
        ].map(s => (
      ...
      <div className="tabs-nav" style={{ marginBottom: 20, maxWidth: 600 }}>
        ...
        <button className={`tab-btn ${activeTab === 'posts' ? 'active' : ''}`} onClick={() => setActiveTab('posts')}>
          <CheckCircle size={13} /> Posts ({posts.filter(p => p.status === 'approved').length})
        </button>
      ...
      {activeTab === 'posts' && (
        <div className="flex gap-2" style={{ marginBottom: 16 }}>
          ...
          <button 
            className={`btn ${postFilter === 'reported' ? 'btn-primary' : 'btn-ghost'} btn-sm`}
            onClick={() => setPostFilter('reported')}
          >
            Reported posts
          </button>
          <button 
            className={`btn ${postFilter === 'reported_comments' ? 'btn-primary' : 'btn-ghost'} btn-sm`}
            onClick={() => setPostFilter('reported_comments')}
          >
            Reported comments
          </button>
        </div>
          ...
          <div className="card p-6" style={{ maxWidth: 400, width: '90%', border: '1px solid rgba(239,29,55,0.3)', background: 'var(--bg-elevated)', boxShadow: '0 10px 40px rgba(0,0,0,0.5)' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 16 }}>
              <div style={{ width: 40, height: 40, borderRadius: '50%', background: 'rgba(239,29,55,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--red-400)' }}>
                {confirmModal.type === 'delete' ? <Trash2 size={20} /> : <ShieldCheck size={20} />}
              </div>
              <h3 style={{ fontSize: '1.1rem', fontWeight: 600, color: 'var(--text-primary)', margin: 0 }}>
                {confirmModal.type.startsWith('delete') ? 'Delete Content' : 'Dismiss Reports'}
              </h3>
            </div>
